...
|  **GET**  |  `/api/categories/:id`  |  |  `{ "id": id, "name": string, entreprises:[{"id": number, "name": string}]}`  | 200, 404 | Récupérer un entreprise spécifique |
|  **POST**  |  `/api/categories`  |  `{ "name": "string" }`  |  `{ "id": number, "name": string }`  | 201, 400 | Créer un nouveau entreprise |
|  **PUT**  |  `/api/categories/:id`  |  `{ "name": string }`  |  `{ "id": id, "name": string }`  | 200, 400, 403, 404 | Modifier une entreprise |
|  **DELETE**  |  `/api/categories/:id`  |  |  | 200, 403, 404 | Supprimer une entreprise |
---
...
| Verbe | Endpoint | Request Body | Réponse attendue | Codes Status | Fonction |
|--------|-----------|------------------|------------------|---------------|-----------|
|  **GET**  |  `/api/soumissions/entreprises/:entrepriseId`  |  | `["id": id, "questionnaire_id": number, "inputs": [{ "id": number, "name": string, "type": string, "required": boolean, "description": string, "options": [{ "id": number, "name": string, "value": string }]}}]`| 200, 404 | Lister les soumissions d’une entreprise |
